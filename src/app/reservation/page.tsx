...
const PICKUPS = [
  ...
  "Paris City Centre", "Disneyland Paris", "Versailles Palace", "Montmartre", "Eiffel Tower Area",
];
...
const FEATURES = ["Meet & Greet included", "Door-to-door", "Porter service", "Free child seats & boosters"];
...
export default function ReservationPage() {
  ...
  const submit = async () => { setSending(true); await new Promise(r => setTimeout(r, 1400)); setSending(false); setDone(true); };
  ...
  const wrap = (err?: string): React.CSSProperties => ({
  ...
  const err = (msg?: string) => msg
    ? <div style={{ fontSize: 12, color: "#dc2626", marginTop: 4 }}>{msg}</div>
    : null;
  ...
  const counter = (key: keyof TripDetails, min: number, max: number, label: string) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label style={S.label}>{label}</label>
      <div style={{ display: "flex", border: "1.5px solid #e5e7eb", borderRadius: 10, overflow: "hidden", background: "#fff" }}>
        <button type="button" onClick={() => setTrip(t => ({ ...t, [key]: Math.max(min, (t[key] as number) - 1) }))}
        ...
        <button type="button" onClick={() => setTrip(t => ({ ...t, [key]: Math.min(max, (t[key] as number) + 1) }))}
          style={{ width: 48, height: 48, fontSize: 22, background: "none", border: "none", cursor: "pointer", color: "#6b7280" }}>+</button>
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 40, background: "#f4f5f7", minHeight: "60vh" }}>
      <div style={{ maxWidth: 520, width: "100%", background: "#fff", borderRadius: 20, boxShadow: "0 8px 40px rgba(0,0,0,0.10)", padding: "48px 40px", textAlign: "center" }}>
        ...
        <div style={{ background: "#f9fafb", borderRadius: 12, padding: 20, marginBottom: 28, textAlign: "left" }}>
          {[["Route", `${trip.from} → ${trip.to}`], ["Date & Time", `${trip.date} at ${trip.time}`], ["Vehicle", vehicle?.name ?? "—"]].map(([l, v]) => (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 12, fontSize: 18, fontWeight: 800 }}>
            <span>Total</span><span style={{ color: GREEN }}>€{vehicle?.price ?? 0}</span>
          </div>
        </div>
              ...
              <div style={{ borderTop: "1px solid rgba(255,255,255,.1)", paddingTop: 18, marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 800, color: "#4ade80", marginTop: 2 }}>€{vehicle.price}</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <main className="rp-body">
            ...
            <div className="rp-card">
              ...
              <div style={{ marginBottom: 32 }}>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: GREEN, marginBottom: 8 }}>Step {step + 1} of {STEPS.length}</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {field(<>
                    ...
                    <div className="rp-input-wrap" style={wrap(errors.from)}>
                      <select value={trip.from} onChange={e => setTrip(t => ({ ...t, from: e.target.value }))} style={S.select}>
                  ...
                  {field(<>
                    ...
                    <div className="rp-input-wrap" style={wrap(errors.to)}>
                      <select value={trip.to} onChange={e => setTrip(t => ({ ...t, to: e.target.value }))} style={S.select}>
                  ...
                  <div className="rp-grid">
                    {field(<>
                      ...
                      <div className="rp-input-wrap" style={wrap(errors.date)}>
                        <input type="date" value={trip.date} onChange={e => setTrip(t => ({ ...t, date: e.target.value }))} style={S.input} />
                      </div>
                      {err(errors.date)}
                    </>)}
                    {field(<>
                  ...
                  <div className="rp-grid">
                    {counter("passengers", 1, 8, "Passengers")}
                    {counter("kids", 0, 6, "Children (under 12)")}
                    {counter("bags", 0, 10, "Luggage Bags")}
                  </div>
                </div>
              )}
              ...
              {step === 1 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  {VEHICLES.map(v => {
                    ...
                    return (
                      <div key={v.id} className={`rp-vehicle${sel ? " sel" : ""}${exc ? " exceeded" : ""}`}
                        ...
                        <div style={{ flex: 1, padding: "14px 18px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12, paddingTop: 10, borderTop: "1px solid #f3f4f6" }}>
                            <div>
                              <div style={{ fontSize: 22, fontWeight: 900, color: DARK, lineHeight: 1 }}>{v.price} <span style={{ fontSize: 13, fontWeight: 700 }}>EUR</span></div>
                              <div style={{ fontSize: 10, color: "#9ca3af", marginTop: 2 }}>✓ Includes VAT & fees</div>
                            </div>
                            {exc ? (
                              ...
                              <button type="button" onClick={e => { e.stopPropagation(); setVehicleId(v.id); }}
              ...
              {step === 2 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {field(<>
                    <label style={S.label}>Full Name <span style={{ color: GREEN }}>*</span></label>
                    <div className="rp-input-wrap" style={wrap(errors.name)}>
                      <input type="text" placeholder="e.g. John Smith" value={personal.name} onChange={e => setPersonal(p => ({ ...p, name: e.target.value }))} style={S.input} />
                    </div>
                    {err(errors.name)}
                  </>)}
                  {field(<>
                    <label style={S.label}>Country <span style={{ color: GREEN }}>*</span></label>
                    <div className="rp-input-wrap" style={wrap(errors.country)}>
                      <select value={personal.country} onChange={e => setPersonal(p => ({ ...p, country: e.target.value }))} style={S.select}>
                  ...
                  <div className="rp-grid">
                    {field(<>
                      ...
                      <div className="rp-input-wrap" style={wrap(errors.whatsapp)}>
                        <input type="tel" placeholder="+33 6 12 34 56 78" value={personal.whatsapp} onChange={e => setPersonal(p => ({ ...p, whatsapp: e.target.value }))} style={S.input} />
                      </div>
                      {err(errors.whatsapp)}
                      <span style={{ fontSize: 11, color: "#9ca3af", marginTop: 3 }}>Include country code, e.g. +33</span>
                    </>)}
                    {field(<>
                      <label style={S.label}>Email Address <span style={{ color: GREEN }}>*</span></label>
                      <div className="rp-input-wrap" style={wrap(errors.email)}>
                        <input type="email" placeholder="you@example.com" value={personal.email} onChange={e => setPersonal(p => ({ ...p, email: e.target.value }))} style={S.input} />
                      </div>
                      {err(errors.email)}
                    </>)}
                  </div>
                  {field(<>
                    <label style={S.label}>Special Requests / Notes <span style={{ fontSize: 12, color: "#9ca3af", fontWeight: 400 }}>(optional)</span></label>
                    <textarea rows={4} placeholder="Flight number, special assistance, child seat size…" value={personal.notes}
                      onChange={e => setPersonal(p => ({ ...p, notes: e.target.value }))}
                      style={{ width: "100%", border: "1.5px solid #e5e7eb", borderRadius: 10, padding: "12px 14px", fontSize: 14, color: DARK, resize: "none", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
                  </>)}
                </div>
              )}
              ...
              {step === 3 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <SummaryCard title="Trip Details" rows={[
                    ["Pickup", trip.from], ["Drop-off", trip.to],
                  ...
                  <div style={{ background: DARK, borderRadius: 14, padding: "18px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ fontSize: 34, fontWeight: 900, color: "#4ade80" }}>€{vehicle?.price ?? 0}</div>
                  </div>
                </div>
              )}
